    ...
    for (index, subject) in enumerate(subjects):
        ...
        for event_type in ['chartevents', 'labevents', 'datetimeevents', 'outputevents', 'noteevents', 'inputevents_cv', 'inputevents_mv', 'procedureevents_mv', 'prescriptions']:
            ...
            if not os.path.exists(args.subjects_root_path / subject / events_file):
                continue
            events_df = pd.read_csv(os.path.join(args.subjects_root_path, subject, events_file), index_col=False,
                                    dtype={'HADM_ID': str, "ICUSTAY_ID": str})

            events_df.columns = events_df.columns.str.upper()
            original_columns = list(events_df.columns.str.upper())
            ...
            to_write = merged_df[sorted(list(set(original_columns) | {'ICUSTAY_ID'}))]
            to_write.to_csv(os.path.join(args.subjects_root_path, subject, events_file), index=False)
